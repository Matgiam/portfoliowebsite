      ...
      <div className="contact-grid" style={{ position: 'relative', display: 'grid', gridTemplateColumns: 'minmax(0,1fr) minmax(0,0.95fr)', gap: 'clamp(32px,5vw,90px)', alignItems: 'start' }}>
        <div style={{ display: 'flex', flexDirection: 'column', gap: 'calc(var(--space-8) * 1.4)' }}>
          ...
          <div style={{ display: 'flex', flexWrap: 'wrap', gap: 'var(--space-4)' }}>
            <a className="btn btn-secondary" data-magnetic href="https://www.linkedin.com/" target="_blank" rel="noopener" style={{ fontSize: 13, padding: 'var(--space-4) var(--space-6)', minHeight: 44, color: 'var(--color-text)' }}>LinkedIn</a>
            <a className="btn btn-secondary" data-magnetic href="https://github.com/MatteoGiambarresi" target="_blank" rel="noopener" style={{ fontSize: 13, padding: 'var(--space-4) var(--space-6)', minHeight: 44, color: 'var(--color-text)' }}>GitHub</a>
          </div>
        </div>

        <form ref={formRef} data-reveal onSubmit={sendMail} style={{ display: 'flex', flexDirection: 'column', gap: 'var(--space-6)', padding: 'calc(var(--space-8) * 1.2)', borderRadius: 'var(--radius-lg)', background: 'var(--color-surface)', boxShadow: 'var(--shadow-sm)' }}>
          ...
          <div className="field">
            ...
            <textarea className="input" id="c-message" name="message" rows={6} placeholder="Tell me about the project, the team, the timing." required />
          </div>
          <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: 'var(--space-4)', flexWrap: 'wrap' }}>
            <span style={{ fontSize: 12, color: 'color-mix(in srgb, var(--color-text) 58%, transparent)' }}>{status}</span>
            <button className="btn btn-primary" data-magnetic type="submit" style={{ fontSize: 13, padding: 'var(--space-4) var(--space-8)', minHeight: 44 }}>
              Send message
            </button>
          </div>
        </form>
